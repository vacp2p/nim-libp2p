...
def plot_metrics(data, title, output_path):
    ...
    # Network Throughput
    ax3.plot(time_points, download_MBps, 'c-', label=f'Download (MB/s)\nmedian = {download_MBps_median:.2f} MB/s\nmax = {download_MBps_max:.2f} MB/s', linewidth=2)
    ax3.plot(time_points, upload_MBps, 'r-', label=f'Upload (MB/s)\nmedian = {upload_MBps_median:.2f} MB/s\nmax = {upload_MBps_max:.2f} MB/s', linewidth=2)
...
if __name__ == "__main__":
    # Example usage: plot all node CSVs in the output directory
    csv_files = glob.glob("performance/output/docker_stats__node-*.csv")
    for csv_file in csv_files:
        node_name = os.path.splitext(os.path.basename(csv_file))[0]
        data = parse_csv(csv_file)
        plot_metrics(data, title=node_name, output_path=f"performance/output/{node_name}.png")
