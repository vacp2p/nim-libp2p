...
for ((i = 0; i < num_peers; i++)); do
    ...
    # Run the Docker container with the current hostname and unique PEERNUMBER
    docker run -e PEERSPERPOD="1" -e PEERS="10" -e CONNECTTO="5" -e MSGRATE="1000" -e MSGSIZE="1000" -e PEERNUMBER="$i" --hostname="$hostname" --network="$custom_network_name" dst-test-node &
