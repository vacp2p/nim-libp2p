...
- Code in `except` blocks must always add raised error message, from `e.msg` property or `getCurrentExceptionMsg()` proc, to logs or append it to new error if it is being raised in order to preserve reason that triggered error. Do not do this in case of `CancelledError` as those error messages never add value.
