...
suite "RendezVous":
  ...
  asyncTest "Request locally returns registered peers":
    ...
    let peerRecords = nodes[0].requestLocally[PeerRecord](namespace)
  ...
  asyncTest "Request returns 0 for empty namespace from remote":
    ...
    check (await peerNodes[0].request[PeerRecord](Opt.some(namespace))).len == 0
  ...
  asyncTest "Request returns registered peers from remote":
    ...
    let peerRecords = await peerNodes[0].request[PeerRecord](Opt.some(namespace))
    check:
      peerRecords.len == 1
  ...
  asyncTest "Unsubscribe removes registered peer from remote":
    ...
    check (await peerNodes[0].request[PeerRecord](Opt.some(namespace))).len == 1
  ...
  asyncTest "Consecutive requests with namespace returns peers with pagination":
    ...
    var peerRecords = await peerNodes[0].request[PeerRecord](Opt.some(namespace), 5)
    check:
      peerRecords.len == 5
      peerRecords.allIt(it in data)
    data.keepItIf(it notin peerRecords)

    peerRecords = await peerNodes[0].request[PeerRecord](Opt.some(namespace))
    check:
      peerRecords.len == 6
      peerRecords.allIt(it in data)
  ...
  asyncTest "Request without namespace returns all registered peers":
    ...
    check (await peerNodes[0].request[PeerRecord](Opt.none(string))).len == 10
  ...
  asyncTest "Consecutive requests with namespace keep cookie and retun only new peers":
    ...
    discard await rdv0.request[PeerRecord](Opt.some(namespace))

    await rdv1.advertise(namespace)
    let peerRecords = await rdv0.request[PeerRecord](Opt.some(namespace))
  ...
  asyncTest "Request with namespace pagination with multiple namespaces":
    ...
    var peerRecords = await rdv.request[PeerRecord](Opt.some(namespaceFoo), 2)
    check:
      peerRecords.len == 2
    ...
    peerRecords = await rdv.request[PeerRecord](Opt.some(namespaceFoo), 5)
    check:
      peerRecords.len == 5
      peerRecords.allIt(it in fooRecords)
    fooRecords.keepItIf(it notin peerRecords)
    ...
    peerRecords = await rdv.request[PeerRecord](Opt.some(namespaceFoo))
    check:
      peerRecords.len == 3
      peerRecords.allIt(it in fooRecords)
    fooRecords.keepItIf(it notin peerRecords)
    ...
    # Bar Page 1 with all
    peerRecords = await rdv.request[PeerRecord](Opt.some(namespaceBar), 30)
    check:
      peerRecords.len == 10
    ...
    peerRecords = await rdv.request[PeerRecord](Opt.some(namespaceBar))
    check:
      peerRecords.len == 5
      peerRecords.allIt(
    ...
    # All records
    peerRecords = await rdv.request[PeerRecord](Opt.none(string))
  ...
  asyncTest "Request with namespace with expired peers":
    ...
    check:
      (await peerNodes[0].request[PeerRecord](Opt.some(namespaceFoo))).len == 5
      (await peerNodes[0].request[PeerRecord](Opt.some(namespaceBar))).len == 5
    ...
    checkUntilTimeout:
      ...
      (await peerNodes[0].request[PeerRecord](Opt.some(namespaceFoo))).len == 0
      (await peerNodes[0].request[PeerRecord](Opt.some(namespaceBar))).len == 0
  ...
  asyncTest "Cookie offset is reset to low after flush (returns current entries)":
    ...
    check (await peerNodes[0].request[PeerRecord](Opt.some(namespace))).len == 4
  ...
  asyncTest "Cookie namespace mismatch resets to low (returns peers despite offset)":
    ...
    check (await peerNodes[0].request[PeerRecord](Opt.some(namespace))).len == 3
  ...
  asyncTest "Peer can register to and unsubscribe multiple namespaces":
    ...
    for ns in namespaces:
      check (await requester1.request[PeerRecord](Opt.some(ns))).len == 1

    # Assert request without namespace returns only one Peer
    check (await requester1.request[PeerRecord]()).len == 1
    ...
    for ns in unsubscribedNamespaces:
      check (await requester2.request[PeerRecord](Opt.some(ns))).len == 0

    # Assert still subscribed namespaces return Peer
    for ns in subscribedNamespaces:
      check (await requester2.request[PeerRecord](Opt.some(ns))).len == 1

    # Assert request without namespace still returns only one Peer
    check (await requester2.request[PeerRecord]()).len == 1
