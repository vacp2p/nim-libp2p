...
suite "Advertiser - startAdvertising / stopAdvertising":
  ...
  test "startAdvertising adds service to disco.services":
    let disco = makeMockDiscovery()
    let service = makeServiceInfo()

    let wasPresent = disco.startAdvertising(service)

    check not wasPresent
    check service in disco.services

  test "startAdvertising is idempotent — returns true on second call":
    let disco = makeMockDiscovery()
    let service = makeServiceInfo()

    discard disco.startAdvertising(service)
    let wasPresent = disco.startAdvertising(service)

    check wasPresent
    check service in disco.services

  test "stopAdvertising removes service from disco.services":
    let disco = makeMockDiscovery()
    let service = makeServiceInfo()

    discard disco.startAdvertising(service)
    let wasMissing = disco.stopAdvertising(service)

    check not wasMissing
    check service notin disco.services

  test "stopAdvertising on non-existent service returns true":
    let disco = makeMockDiscovery()
    let service = makeServiceInfo()
