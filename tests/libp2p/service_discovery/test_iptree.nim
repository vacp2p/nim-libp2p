...
suite "IpTree":
  ...
  test "insertIp rejects IPv6 on populated tree":
    ...
    check tree.insertIp(ipv6).isErr()
  ...
  test "removeIp rejects IPv6 on populated tree":
    ...
    check tree.removeIp(ipv6).isErr()
  ...
  test "diverse IP distribution yields low individual scores":
    ...
    discard tree.insertIp(parseIpAddress("10.0.0.1")) # Class A private
    discard tree.insertIp(parseIpAddress("172.16.0.1")) # Class B private
    discard tree.insertIp(parseIpAddress("192.168.0.1")) # Class C private
    discard tree.insertIp(parseIpAddress("8.8.8.8")) # Google DNS
    discard tree.insertIp(parseIpAddress("1.1.1.1")) # Cloudflare DNS
