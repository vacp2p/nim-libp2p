...
suite "MultiAddress IP utilities":
  test "getIp":
    check:
      ...
      ma("/tcp/1234").getIp() == Opt.none(IpAddress)
      ma("/ip4/5.6.7.8/tcp/80/ws").getIp() ==
  ...
  test "replacePort":
    check:
      ...
      # no port component: no-op (callers gate with hasPort if they need to detect)
