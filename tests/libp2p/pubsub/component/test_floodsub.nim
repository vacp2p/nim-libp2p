...
suite "FloodSub Component":
  ...
  asyncTest "FloodSub init takes the overhead rate limit":
    const limit = RateLimit(bytes: 10, interval: 1.hours)
    let node = generateNodes(
      1, overheadRateLimit = Opt.some(limit), disconnectPeerAboveRateLimit = true
    )
      .toFloodSub()[0]

    check:
      node.overheadRateLimit == Opt.some(limit)
      node.disconnectPeerAboveRateLimit

  test "FloodSub init rejects disconnectPeerAboveRateLimit without a limit":
    expect InitializationError:
      discard generateNodes(1, disconnectPeerAboveRateLimit = true)
