...
suite "BufferStream":
  test "push data to buffer":
    proc testPushTo(): Future[bool] {.async.} =
      ...
      let buff = newBufferStream(writeHandler)
      check buff.len == 0
      var data: seq[char]
      data.add(@"12345")
  ...
  test "read":
    proc testRead(): Future[bool] {.async.} =
      ...
      check @"12345" == cast[string](await buff.readOnce())
  ...
  test "readOnce":
    proc testReadOnce(): Future[bool] {.async.} =
      ...
      let readFut = buff.readOnce()
      await buff.pushTo(cast[seq[byte]](@"123"))
      # check buff.len == 3
      let data = await readFut
  ...
  test "shouldn't get stuck on close":
    proc test(): Future[bool] {.async.} =
      ...
      var message = createMessage("MESSAGE", 1024 * 2 + 1)
