...
suite "GossipSub":
  ...
  test "with multiple peers": 
    proc testRun(): Future[bool] {.async.} =
      var passed: int
      proc handler(topic: string, data: seq[byte]) {.async, gcsafe.} =
        echo "YOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO"
        check topic == "foobar"
        passed.inc()
      ...
      for dialer in nodes:
        for node in nodes:
          if dialer.peerInfo.peerId.get().pretty != node.peerInfo.peerId.get().pretty:
            var buf1 = newBufferStream(size = DefaultBufferSize * 100)
            var conn1 = newConnection(buf1)
            conn1.peerInfo = dialer.peerInfo

            var buf2 = newBufferStream(size = DefaultBufferSize * 100)
            var conn2 = newConnection(buf2)
