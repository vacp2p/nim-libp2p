...
type
  ...
  PB* = object ## Use this type to specify Google ProtoBuf's varint encoding
  LP* = object ## Use this type to specify LibP2P varint encoding
  ...
  hint* = distinct int ## Signed integer types which will be encoded using simple cast.
...
template toUleb[T: uint64 | uint32 | uint16 | uint8 | uint](x: T): T =
  x
...
template toUleb(x: hint64): uint64 =
  cast[uint64](x)

template toUleb(x: hint32): uint32 =
  cast[uint32](x)
...
template fromUleb(x: uint64, T: type uint64): T =
  x

template fromUleb(x: uint32, T: type uint32): T =
  x
...
proc putVarint*[T: PB | LP](
  ...
  when vtype is PB:
    when (type(value) is PBSomeSVarint) or (type(value) is PBZigVarint):
      ...
      varintFatal(
        "Protobuf's varint do not support type [" & typetraits.name(type(value)) & "]"
      )
  elif vtype is LP:
    when (type(value) is LPSomeVarint):
      ...
      varintFatal(
        "LibP2P's varint do not support type [" & typetraits.name(type(value)) & "]"
      )
