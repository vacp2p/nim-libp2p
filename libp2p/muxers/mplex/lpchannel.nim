  ...
  if s.isOpen and not s.conn.isClosed:
    ...
    proc resetMessage() {.async: (raises: []).} =
      try:
        ...
        trace "Can't send reset message", s, conn = s.conn, errMsg = exc.msg
        await s.conn.close()
...
method close*(s: LPChannel) {.async: (raises: []).} =
  ...
  if s.isOpen and not s.conn.isClosed:
    try:
      ...
      # likely down already
      await s.conn.close()
      trace "Cannot send close message", s, id = s.id, errMsg = exc.msg
...
proc completeWrite(
  ...
  try:
    ...
    trace "exception in lpchannel write handler", s, errMsg = exc.msg
    await s.reset()
    await s.conn.close()
    raise newLPStreamConnDownError(exc)
  finally:
