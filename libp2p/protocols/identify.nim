...
method init*(p: Identify) =
  proc handle(stream: Stream, proto: string) {.async: (raises: [CancelledError]).} =
    ...
    try:
      ...
      trace "identify handler failed to write message", error = e.msg, stream
    finally:
  ...
  proc handle(stream: Stream, proto: string) {.async: (raises: [CancelledError]).} =
    ...
    var message =
      try:
        ...
        trace "failed to read message from stream", error = e.msg, stream
        return
    ...
    if not handler.isNil:
      ...
      try:
        ...
        trace "got unexpected error", error = e.msg, stream
