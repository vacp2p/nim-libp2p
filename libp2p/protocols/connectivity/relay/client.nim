...
proc sendStopError(
  ...
  try:
    ...
    trace "failed to send stop status", error = e.msg
  ...
  let
    ...
    msg =
      try:
        ...
        trace "error writing or reading reservation message", error = exc.msg
...
proc dialPeerV1*(
  ...
  try:
    ...
    trace "error writing hop request", error = exc.msg
  ...
  let msgRcvFromRelayOpt =
    try:
      ...
      trace "error reading stop response", error = exc.msg
...
proc new*(
  ...
  proc handleStream(
    ...
    try:
      ...
      trace "exception in client handler", error = exc.msg, stream
    finally:
