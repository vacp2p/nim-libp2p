          ...
          for h in m.handlers:
            if (not isNil(h.match) and h.match(ms)) or ms == h.proto:
              ...
              try:
                ...
                # NOTE: the catch all here is for exceptions
                # generated by the user handler above,
                # there isn't anything we can do here but
                # log and cleanup
                # TODO: for some reason `exc.msg` misses the
                # callstack and message. nim bug?
                warn "exception in user protocol", exc = exc.msg
              finally:
                return
