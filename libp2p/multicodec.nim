...
proc parseCodecTuple(node: NimNode): tuple[name: string, code: int] {.compileTime.} =
  ...
  of nnkTupleConstr:
    if node.len != 2:
      error("Expected (name, code) tuple", node)

    let nameNode = node[0]
    let codeNode = node[1]

    if nameNode.kind != nnkStrLit:
      error("Codec name must be string literal", nameNode)
    let name = nameNode.strVal

    if codeNode.kind != nnkIntLit: # and codeNode.kind != nnkHexLit:
      error("Codec code must be integer literal", codeNode)
    let code = codeNode.intVal.int

    result = (name, code)

  else:
    error("Expected (name, code) tuple, got: " & $node.kind, node)
