...
proc sendOne(p: IdentifyPusher, peerId: PeerId) {.async: (raises: [CancelledError]).} =
  ...
  try:
    ...
    trace "failed to open stream for identify push", error = e.msg, peerId
  except MultiStreamError as e:
    trace "multistream negotiation failed for identify push", error = e.msg, peerId
  except LPStreamError as e:
    trace "stream error during identify push", error = e.msg, peerId
  finally:
    if not stream.isNil:
      if pushCompleted:
