...
proc handleUnmap(upnp: Miniupnp, req: UpnpRequest, resp: var UpnpResponse) =
  ...
  let r =
    upnp.deletePortMapping(externalPort = $req.unmapExternal, protocol = toUpnpProto(req.unmapProto))
  if r.isErr:
...
method discover*(
  ...
  await self.dispatch(UpnpRequest(kind: urDiscover, discoverDelayMs: cint(timeout.milliseconds)))
...
method close*(self: UpnpMapper) {.gcsafe, raises: [].} =
  ...
  let fr = self.ctx.reqSignal.fireSync()
  if fr.isErr or not fr.get():
    warn "UpnpMapper shutdown signal failed", err = (if fr.isErr: fr.error else: "timeout")
