...
proc load_3(inp: openArray[byte]): uint64 =
  result = uint64(inp[0])
  result = result or (uint64(inp[1]) shl 8)
  result = result or (uint64(inp[2]) shl 16)

proc load_4(inp: openArray[byte]): uint64 =
  result = uint64(inp[0])
  result = result or (uint64(inp[1]) shl 8)
  result = result or (uint64(inp[2]) shl 16)
  result = result or (uint64(inp[3]) shl 24)
...
proc geFromBytesNegateVartime(h: var GeP3, s: openArray[byte]): int32 =
  ...
  if feIsNegative(h.x) == int32(s[31] shr 7):
    feNeg(h.x, h.x)
