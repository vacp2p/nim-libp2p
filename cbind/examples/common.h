...
static inline void on_bool(int ec, const bool *reply, const char *em,
                           void *ud) {
  (void)reply;
...
// `w` is evaluated more than once, so pass a plain lvalue (never
// `waiters[i++]`).
...
static inline void on_created(int ec, LibP2PCtx *ctx, const char *em,
                              void *ud) {
...
// releases it with libp2p_ctx_destroy; NULL on failure (message already
// logged).
static inline LibP2PCtx *await_create(const Libp2pConfig *cfg,
                                      const char *label) {
  CreateWaiter w;
  memset(&w, 0, sizeof(w));
...
// so peerinfo is copied into caller-owned buffers here. Callers that forward
// the copied addrs (e.g. into a ConnectRequest) rely on this same cap, so it
// lives here rather than as a bare literal.
#define MAX_ADDRS 16
typedef struct {
  atomic_int done;
  int err_code;
...
static inline void on_peerinfo(int ec, const PeerInfoResponse *reply,
...
static inline bool await_peerinfo(LibP2PCtx *ctx, PeerInfoWaiter *w,
                                  const char *label) {
  memset(w, 0, sizeof(*w));
