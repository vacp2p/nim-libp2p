...
import libp2p/discovery/discoverykad
...
##
## Note that other discovery methods such as [Rendezvous](https://github.com/libp2p/specs/blob/master/kad-dht/README.md) or [discv5](https://github.com/ethereum/devp2p/blob/master/discv5/discv5.md) exist.
...
proc main() {.async.} =
  ...
  dm.add(KadDiscovery.new(kad, ttr = 250.milliseconds))

  # Use the discovery manager to find peers on the OddClub topic to greet them
  let queryOddClub = dm.request(KadNamespace("OddClub"))
  for _ in 0 .. 2:
    let
  ...
  let queryEvenGang = dm.request(KadNamespace("EvenGang"))
  for _ in 0 .. 2:
    let
